...
//     return messageLines.map((line, index) => (
...
//           {message.user === "gpt" && <svg xmlns="http://www.w3.org/2000/svg" width={40} height={40}>
...
function ChatMessage({ message }) {
  const containsCodeKeywords = /source code|code|program/i.test(message.message);

  const renderMessage = () => {
    if (containsCodeKeywords) {
      // Render source code in a new div with styling
      return (
        <div style={{ backgroundColor: 'black', border: '2px solid red', padding: '8px', borderRadius: '4px' }}>
          {message.message}
        </div>
      );
    } else {
